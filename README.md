...
- 📑 **รายงานการเงินของนิติ** — ลิงก์ dashboard การเงิน 2 รายการ
  (รายงานระหว่างเดือน = Looker Studio, รายงานเดือนล่าสุด = Power BI)
- 💳 **วิธีชำระเงิน** — เลขบัญชีและช่องทางการชำระ
- 📞 **ติดต่อกรรมการ** — เบอร์ติดต่อและกลุ่มไลน์ลูกบ้าน
- 📱 **Rich Menu** — เมนู 6 ปุ่ม (3×2) ด้านล่างห้องแชต ใช้งานง่ายไม่ต้องจำคำสั่ง
  (ปุ่มขวาล่างเว้นว่างไว้สำหรับฟีเจอร์ในอนาคต)
...
### การติดตั้ง Rich Menu (เมนู 6 ปุ่ม 3×2)
1. อัปโหลดรูปเมนู (PNG/JPEG ขนาด **1250×843**, < 1 MB) ขึ้น Google Drive
   — ต้องเป็นขนาดนี้พอดี ไม่งั้น LINE จะตอบ `413 Request Entity Too Large`
2. ตั้งค่า Script Property `RICHMENU_IMAGE_FILE_ID` = file ID ของรูป
   (คัดลอกจากลิงก์ Drive: ส่วนระหว่าง `/d/` กับ `/view`)
3. รันฟังก์ชัน `setupRichMenu()` ใน Apps Script editor (อนุญาตสิทธิ์ Drive)
   — log ควรขึ้น `create → 200`, `image → 200`, `setDefault → 200`
4. จัดการเมนูเดิมได้ด้วย `listRichMenus()` และ `deleteRichMenu(id)`
   (หรือ `deleteOldRichMenu()` = wrapper ให้วาง id ลงในโค้ดแล้วกด Run)

> **Layout ปัจจุบัน (3 คอลัมน์ × 2 แถว):**
> เช็คยอดบ้าน · รายงานล่าสุด · รายงานการเงินของนิติ /
> วิธีชำระเงิน · ติดต่อกรรมการ · *(ว่าง — สำรอง)*
